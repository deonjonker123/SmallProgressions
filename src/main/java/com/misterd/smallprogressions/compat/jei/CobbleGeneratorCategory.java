...
public class CobbleGeneratorCategory implements IRecipeCategory<CobbleGenRecipe> {
    ...
    @Override
    public RecipeType<CobbleGenRecipe> getRecipeType() { return RECIPE_TYPE; }

    @Override
    public Component getTitle() { return Component.translatable("jei.smallprogressions.cobble_generation"); }
    ...
    public void draw(CobbleGenRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphicsExtractor guiGraphics, double mouseX, double mouseY) {
        ...
        int centerX = 59;
        guiGraphics.text(font, timeText, centerX - font.width(timeText) / 2, 45, 0x808080, false);
        guiGraphics.text(font, tickText, centerX - font.width(tickText) / 2, 54, 0x606060, false);
    }
}