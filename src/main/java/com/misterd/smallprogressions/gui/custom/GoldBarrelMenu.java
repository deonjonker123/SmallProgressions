...
public class GoldBarrelMenu extends AbstractContainerMenu {
    public final GoldBarrelBlockEntity blockEntity;
    private final Level level;

    private static final int TE_SLOTS = 72;
    private static final int TE_FIRST = 0;
    private static final int PLAYER_FIRST = TE_SLOTS;
    private static final int PLAYER_SLOTS = 36;

    public GoldBarrelMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public GoldBarrelMenu(int containerId, Inventory inv, BlockEntity blockEntity) {
        super(SPMenuTypes.GOLD_BARREL_MENU.get(), containerId);
        this.blockEntity = (GoldBarrelBlockEntity) blockEntity;
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, SPBlocks.GOLD_BARREL.get());
    }

    private void addPlayerInventory(Inventory inv) {
        for (int i = 0; i < 3; i++)
            for (int l = 0; l < 9; l++)
                addSlot(new Slot(inv, l + i * 9 + 9, 8 + l * 18, 173 + i * 18));
    }

    private void addPlayerHotbar(Inventory inv) {
        for (int i = 0; i < 9; i++)
            addSlot(new Slot(inv, i, 8 + i * 18, 232));
    }

    private static class BESlot extends Slot {
        private final GoldBarrelBlockEntity be;
        private final int index;

        BESlot(GoldBarrelBlockEntity be, int index, int x, int y) {
