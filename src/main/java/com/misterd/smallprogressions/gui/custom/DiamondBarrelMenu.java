...
public class DiamondBarrelMenu extends AbstractContainerMenu {
    ...
    private static final int TE_SLOTS = 104;
    ...
    public DiamondBarrelMenu(int containerId, Inventory inv, BlockEntity blockEntity) {
        ...
        for (int row = 0; row < 8; row++)
            for (int col = 0; col < 13; col++)
                addSlot(new BESlot(this.blockEntity, col + row * 13, 12 + col * 18, 18 + row * 18));
    ...
    private void addPlayerInventory(Inventory inv) {
        for (int i = 0; i < 3; i++)
            for (int l = 0; l < 9; l++)
                addSlot(new Slot(inv, l + i * 9 + 9, 48 + l * 18, 173 + i * 18));
    }

    private void addPlayerHotbar(Inventory inv) {
        for (int i = 0; i < 9; i++)
            addSlot(new Slot(inv, i, 48 + i * 18, 232));
    }

    private static class BESlot extends Slot {
        private final DiamondBarrelBlockEntity be;
        private final int index;

        BESlot(DiamondBarrelBlockEntity be, int index, int x, int y) {
